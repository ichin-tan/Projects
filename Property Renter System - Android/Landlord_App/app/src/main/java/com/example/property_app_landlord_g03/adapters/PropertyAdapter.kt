...
class PropertyAdapter(var myItems:List<Property>, val clickInterface: ClickDetectorInterface) : RecyclerView.Adapter<PropertyAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.switchAvailable.isChecked = property.isAvailable
        ...
        holder.binding.btnDelete.setOnClickListener {
            clickInterface.deleteProperty(position)
        }

        holder.binding.btnUpdate.setOnClickListener {
            clickInterface.updateProperty(position)
        }

        holder.binding.switchAvailable.setOnCheckedChangeListener { _, switchValue: Boolean ->
            clickInterface.updateAvailableSwitch(position, switchValue)
        }
    }
}